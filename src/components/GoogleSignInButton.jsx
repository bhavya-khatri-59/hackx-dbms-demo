...
const StudentDetailsModal = ({ open, onClose, userProfile }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Check your registration number or phone number and try again");
      }

      login(responseData); // Update global state
      onClose();
      navigate('/dashboard');

    } catch (error) {
      console.error("Submission Error:", error);
      window.alert("Error submitting details: " + error.message);
...
const GoogleSignInButton = () => {
  ...
  const getButtonText = () => {
      ...
      if (isGoogleReady) return 'Sign in with Google';
      return 'Loading...';
  };
  ...
  return (
    <>
      <motion.button
        ...
        className={`
          inline-flex items-center justify-center space-x-2 px-8 py-3 
